...
<body>
<form action="reviewinsert.do?seq=${info.id_store}" method="post" enctype="Multipart/form-data">
	<div id="reviewWord">
		...
		<p style="position: absolute; left: 5%;">${info.store_name}</p>
		<div class="greyRule"><hr /></div>
		<img id="reviewProfileImage" src="<%=session.getAttribute("kakaoImage")%>" alt="" style="position: absolute; top: 50px; left: 5%; width: 50px; height: 50px;">
		<input id="reviewPoint" value="${info.rating}" disabled="disabled" style="background-color:white; position: absolute; top: 50px; right: 5%; width: 50px; border:0px; height: 50px; font-size: 25px; text-align: center;">
		<p id="reviewProfileName" style="position: absolute; top: 40px; left: 20%; font-weight: bold;"><%=session.getAttribute("kakaoName")%></p>
		<a href="restaurants.do?seq=${seq}" style="position: absolute; top: 120px; left: 3%;">리뷰의 가게로 이동</a>
		<div style="position: absolute; background-color:white; border:0px; width: 92%; top: 150px; left: 3%; resize: none; word-wrap:normal; word-break:break-all; overflow-y:auto;">					
				<input type="hidden" name ="kakaoName" value="<%=session.getAttribute("kakaoId")%>">
				<input type="hidden" name ="kakaoName" value="<%=session.getAttribute("kakaoName")%>">
				<input type="hidden" name ="kakaoName" value="<%=session.getAttribute("kakaoImage")%>">
				<input type="hidden" name ="store_name" value="${info.store_name}">
				<input type="hidden" name ="store_id" value="${info.id_store}">
				<input type="text" name ="reviewcontent" style="width: 98%; height:500px;"><br>
				<input id="upload" multiple="multiple" type="file" name="imgs"><br><br>
